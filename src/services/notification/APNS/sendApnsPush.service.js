...
// en utilisant la clé .p8 (la même que celle uploadée dans Firebase).
...
const getProvider = () => {
  ...
  if (process.env.APNS_KEY_CONTENT) {
    // Restaure les sauts de ligne échappés (`\n` littéral → vrai newline)
...
const sendApnsPush = async ({ tokens, title, body, data = {} }) => {
  ...
  const bundleId = process.env.APNS_BUNDLE_ID || 'com.moobilpay.com';
  ...
  try {
    ...
    response.failed.forEach((fail) => {
      ...
      // Tokens définitivement invalides → à supprimer de la BD
